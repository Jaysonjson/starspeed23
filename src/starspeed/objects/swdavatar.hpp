...
namespace StarSpeed {
	...
	public:
        ...
        void update() override {
            if(done_) {
                ...
                if (hueTimer == 16) {
                    ...
                    getComponent<Motor::SpriteComponent>(7)->customColor_.set((int) dist(mt), (int) dist(mt),
                                                                              (int) dist(mt), 255);
                    hueTimer = 0;
                }
            }
		...
		void onCreate() override {
            std::thread thread = std::thread(&SWDAvatar::onCreateThread, this);
            thread.detach();
		}
        ...
        void onCreateThread() {
            ...
            if (f.good()) {
                Motor::DynamicTexture* SWD_AVATAR_TEX = new Motor::DynamicTexture(Motor::Path::docs + "swd_avatar.png");
                SWD_AVATAR_TEX->loadAsThread();
                addComponent<Motor::DynamicSpriteComponent>(SWD_AVATAR_TEX);
